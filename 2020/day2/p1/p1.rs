...
fn main() {
    ...
    */
    ...
    for word in passwords {
        // Get count of all occurances of substring, in this case, the letter
        let occurrence = word.password.matches(word.letter).count();
        if occurrence >= word.occurrences.0 && occurrence <= word.occurrences.1 {
            valid_count +=1 ;
        }
    }
